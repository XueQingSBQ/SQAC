  ...
  ans.resize(1);
   for(auto label:SULabel)
   {
     for(auto label2:SVLabel){
        ...
        graphinc g=mac.online_core(q,u_set,v_set,edge_u,edge_v,a,b);
        if(g.u_set.size()>0)
        {
   ...
   while (ans[l].size()!=0)
   {
      ...
      for(int i=0;i<ans[l].size();i++){
        ...
        for(int j=i+1;j<ans[l].size();j++){
           ...
           g=mac.online_core(q,g.u_set,g.v_set,edge_u,edge_v,a,b);
...
double inc_opt(int q,int a,int b,Graph&G,MAC_index&mac)
{
   auto start= chrono::system_clock::now();
    ...
    for(auto label:SULabel){
        ...
        if(mac.MAC_u_uindex[umask].find(a)!=mac.MAC_u_uindex[umask].end()){
           vector<u_node*>& u_label_nodes=mac.MAC_u_uindex[umask][a];
           if(u_label_nodes.size()>b){
              u_node* u_label_node=u_label_nodes[b];
              while(u_label_node!=nullptr){
                 unordered_set<int>& uset=u_label_node->uset;
    ...
    for(auto label:SVLabel){
        ...
        if(mac.MAC_v_vindex[vmask].find(b)!=mac.MAC_v_vindex[vmask].end()){
           vector<u_node*>& v_label_nodes=mac.MAC_v_vindex[vmask][b];
           if(v_label_nodes.size()>a){
              u_node* v_label_node=v_label_nodes[a];
              while ( v_label_node!=nullptr)
              {
