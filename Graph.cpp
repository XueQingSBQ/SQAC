...
//                {
//                   auto start = chrono::system_clock::now();
//                   computeBetaMax_VE(node_data.edge_mp_u,node_data.edge_mp_v,node_data.u_set,node_data.v_set,node_data.degree_u,node_data.degree_v,u_a_Bmax,v_b_Amax,i,n1,n2);
...
//                {
//                   auto start = chrono::system_clock::now();
//                   computeAlphaMax_VE(node_data.edge_mp_u,node_data.edge_mp_v,node_data.u_set,node_data.v_set,node_data.degree_u,node_data.degree_v,u_a_Bmax,v_b_Amax,i,n1,n2);
//                   auto end= chrono::system_clock::now();
...
//    // save_VC_index("VC_v_u.txt",VC_v_uindex);
//    // save_VC_index("VC_u_v.txt",VC_u_vindex);
...
//    file.close();
// }
// void Graph::save_index_batch(const string& filename, vector<string>label_u,vector<uint32_t> label_u_combinations,  const unordered_map<int, unordered_map<int, vector<label_node*>>>& index_batch) {
//    ofstream file(filename);
//    if (!file.is_open()) {
//        cerr << "Error: Failed to open file " << filename << " for writing.\n";
//        return;
//    }
//    unordered_map<uint32_t, vector<string>> mask_to_labels_map;
//    for (auto& combination : label_u_combinations) {
//        vector<string> temp = mask_to_labels(combination, label_u);
//        mask_to_labels_map[combination] = temp;
//    }
//    auto write_label_vector = [&](const vector<label_node*>& vec) {
//        file << "[";
...
//                for (auto& lbl : vec[i]->label_vector) {
//                    //file << bitset<4>(lbl) << ",";
//                    for(auto& s : mask_to_labels_map[lbl]){
//                        file << s ;
//                    }
//                    file << ",";
//                }
